...
const SalesLedger: React.FC<SalesLedgerProps> = () => {
    ...
    const [editedData, setEditedData] = useState<SalesLedgerEntry | null>(null);

    const handleEdit = (item: SalesLedgerEntry) => {
    ...
    const handleEditChange = (field: keyof SalesLedgerEntry, value: any) => {
        if (editedData) {
    ...
    const handleAddRow = () => {
        ...
        const newItem: SalesLedgerEntry = {
            id: newId,
            // FIX: Changed `date` to `orderDate` to match the SalesLedgerEntry type.
        ...
        <div className="overflow-x-auto">
            ...
            <table className="w-full min-w-[800px] text-sm text-left text-brand-text-secondary">
                <thead className="text-xs text-brand-text-primary uppercase bg-gray-700/50">
                    <tr>
                        {/* FIX: Changed table header from "Date" to "Order Date". */}
                        <th className="p-2">Order Date</th>
                        <th className="p-2">SKU</th>
                        <th className="p-2">Units</th>
                        <th className="p-2">Unit Price (€)</th>
                        <th className="p-2">Channel</th>
                        <th className="p-2">Support Donation (€)</th>
                        ...
                        ledgerData.map(item => {
                            ...
                            return isEditing && editedData ? (
                                <tr key={item.id} className="bg-gray-700/50">
                                    {/* FIX: Changed `editedData.date` to `editedData.orderDate` and the field in `handleEditChange`. */}
                                ...
                                <tr key={item.id} className="border-b border-brand-border hover:bg-gray-700/30">
                                    {/* FIX: Changed `item.date` to `item.orderDate`. */}
