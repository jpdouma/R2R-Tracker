...
const CashJournal: React.FC<CashJournalProps> = () => {
    ...
    const { editingRowId, editedData, handleEdit, handleCancel, handleEditChange, setEditedData } = useEditableRow<CashJournalEntry>();
    ...
    // Custom change handler for complex state updates within the form
    const customHandleChange = (field: keyof CashJournalEntry, value: any) => {
       if (editedData) {
                            ...
                            return isEditing && editedData ? (
                               <tr key={item.id} className="bg-gray-700/50">
                                    ...
                                    <td className="p-1 w-48">
                                         <select value={editedData.category} onChange={e => customHandleChange('category', e.target.value)} className="bg-brand-surface p-2 rounded-md w-full text-xs outline-none focus:ring-1 focus:ring-brand-primary">
                                    ...
                                    <td className="p-1 text-right" colSpan={2}>
                                        <div className="flex gap-2">
                                            <select value={editedData.type} onChange={e => customHandleChange('type', e.target.value)} className="bg-brand-surface p-2 rounded-md w-full text-xs outline-none focus:ring-1 focus:ring-brand-primary">
